...
#include "../include/cloud_data_color.hpp"
...
int main(int argc, char** argv) {
    ...
    for(size_t i = 0; i < (data.size()-1); ++i)
    {
        ...
        *local_map_ptr += *transformed_cloud;                       
        ...
        //pcl::io::savePCDFileBinary((fmt%"local_frame_"%(i+1)%"pcd").str(), *local_map_ptr);

        std::cout << "for : " << local_map_ptr -> size() << std::endl;
    ...
    *local_map_ptr += *result;
    std::cout << "END : " << local_map_ptr -> size() << std::endl;
    pcl::io::savePCDFileBinary("map_color_icp.pcd", *local_map_ptr);
    return 0;
}