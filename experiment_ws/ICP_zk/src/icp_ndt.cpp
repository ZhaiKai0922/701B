...
int main(int argc, char** argv) {
    ...
    for(size_t i = 0; i < (data.size()-1); ++i)
    {
        ...
        step_pose = last_pose.inverse() * final_pose;
        predict_pose = final_pose * step_pose;
        last_pose = final_pose;
    }
    *local_map_ptr += *result_icp;
    std::cout << "END : " << local_map_ptr -> size() << std::endl;
    pcl::io::savePCDFileBinary("icp_ndt.pcd", *local_map_ptr);
    return 0;
    p-> spin();
    return 0;
