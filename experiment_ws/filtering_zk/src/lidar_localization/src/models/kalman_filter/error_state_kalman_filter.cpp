...
void ErrorStateKalmanFilter::Init(const Eigen::Vector3d &vel,
  ...
  linear_acc_init = linear_acc_init - accl_bias_;  //body系下
...
bool ErrorStateKalmanFilter::GetVelocityDelta(
  ...
  linear_acc_mid = 0.5 * (a_curr + a_prev); //w系下的linear_acc_mid
  velocity_delta = T * linear_acc_mid;

  linear_acc_mid = 0.5 * (linear_acc_curr + linear_acc_prev) - accl_bias_;  //b系下的linear_acc_mid
...
void ErrorStateKalmanFilter::EliminateError(void) {
  ...
  // c. orientation:
  Eigen::Matrix3d dtheta_cross =
      Sophus::SO3d::hat(X_.block<3, 1>(INDEX_ERROR_ORI, 0));
  pose_.block<3, 3>(0, 0) = pose_.block<3, 3>(0, 0) * (Eigen::Matrix3d::Identity() - dtheta_cross); // fix this
