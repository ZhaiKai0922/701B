...
public:
	...
	virtual void computeError() override {
		...
		_measurement.block<3, 1>(INDEX_P, 0) = alpha_ij;
        _measurement.block<3, 1>(INDEX_R, 0) = theta_ij;
        _measurement.block<3, 1>(INDEX_V, 0) = beta_ij;
		...
		_error.block<3, 1>(INDEX_R, 0) = 2 * (Sophus::SO3d::exp(theta_ij).inverse() * ori_i.inverse() * ori_j).log();
